def _deno_binary_impl(ctx):
    ...
    deno_dir = ctx.bin_dir.path + path_separator
    deno_dir += "%s_deno_dir" % ctx.attr.name

    ctx.actions.write(
        output = outfile,
        content = """
        {export_cmd} DENO_DIR="{deno_dir}"
        {deno_path} run {flags} {main_file} {runtime_args}
        """.format(
            deno_dir = deno_dir,
